...
const RepaymentsTable = () => {
  ...
  const deleteRepayment = async(ID) => {
    const isConfirmed = window.confirm('Are you sure you want to delete this repayment?');
    ...
    try {
      const response = await axios.delete(`http://localhost:5500/repayments/delete/${ID}`);
  ...
  useEffect(() => {
    const fetchRepayments = async () => {
      try {
        const response = await axios.get('http://localhost:5500/repayments/get');
  ...
  useEffect(() => {
    ...
    setFilteredRepayments(
      repayments.filter((repayment) => {
        const loanIdMatch = repayment.LoanID.toString().includes(lowerCaseQuery);
        const dateMatch = repayment.PaymentDate.toString().toLowerCase().includes(lowerCaseQuery);
  ...
  return (
    <>
      <div className="flex flex-col container mx-auto py-6 font-mono">
              ...
              <thead>
                <tr className="text-xs font-semibold tracking-wide text-gray-900 bg-gray-100 uppercase border-b border-gray-600">
                 {/* <th className="px-2 py-2 border">Actions</th> */}
                  ...
                  <th className="px-2 py-2 border">Payment Amount</th>
                  <th className="px-2 py-2 border">Payment Method</th>
                  <th className="px-2 py-2 border">Remaining Balance</th>
                  <th className="px-2 py-2 border">Delinquency Days</th>
                  <th className="px-2 py-2 border">Principal Paid</th>
                  <th className="px-2 py-2 border">Interest Paid</th>
                </tr>
              </thead>
                    ...
                    {/* <td className="px-2 py-2 border">
                      <div className="flex space-x-2 justify-center">
                        <BsPencilFill
                          ...
                          onClick={() => openPopup(repayment)}
                        />
                        <FaTrashAlt
                          className="hover:scale-110 transition-transform duration-200 cursor-pointer"
                          onClick={() => deleteRepayment(repayment.RepaymentID)}
                        />
                      </div>
                    </td> */}
                    <td className="px-2 py-2 border">{repayment.RepaymentID}</td>
                    <td className="px-2 py-2 border">{repayment.LoanID}</td>
                    <td className="px-2 py-2 border">{formatDate(repayment.PaymentDate)}</td>
                    <td className="px-2 py-2 border">{repayment.PaymentAmount}</td>
                    <td className="px-2 py-2 border">{repayment.PaymentMethod}</td>
                    <td className="px-2 py-2 border">{repayment.RemainingBalance}</td>
                    <td className="px-2 py-2 border">{repayment.DelinquencyDays}</td>
                    <td className="px-2 py-2 border">{repayment.PrincipalPaid}</td>
                    <td className="px-2 py-2 border">{repayment.InterestPaid}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {filteredRepayments.length > rowsToShow && (
      ...
      {isPopupOpen && <EditRepaymentPopup repayment={selectedRepayment} onClose={closePopup} />}
    </>
  );
};

const EditRepaymentPopup = ({ repayment, onClose }) => {
  const navigate = useNavigate()
  const [loans, setLoans] = useState([])
  const formik = useFormik({
    initialValues: repayment,
    onSubmit: async (values) => {
      const isConfirmed = window.confirm('Are you sure you want to update this repayment?');
      ...
      try {
        await axios.put(`http://localhost:5500/repayments/update/${repayment.RepaymentID}`, values);
  ...
  return (
    <div className="fixed inset-0 font-mono flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
      ...
      >
        <h2 className="text-xl font-semibold mb-4">Edit Repayment</h2>
        <div className="lg:col-span-2">
                ...
                <div className="md:col-span-5">
                  ...
                  <div className="md:col-span-2">
                    <label htmlFor="RepaymentValue">Repayment Value</label>
                    <input
                      type="number"
                      name="RepaymentValue"
                      id="RepaymentValue"
                  ...
                  <div className="md:col-span-2">
                    <label htmlFor="RepaymentDescription">Repayment Description</label>
                    <input
                      type="text"
